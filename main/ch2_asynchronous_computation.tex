...
composition. One of the basic combinators is \verb|map|, which, given a future
and a mapping function for the value of the future, produces a new future that
is completed with the mapped value once the original future is successfully
completed. What more, due to the type signature of these methods (they each
return another \verb|Future|), it's possible to {\em compose} them.

\begin{itemize}
  \itemsep0em
  \item \verb|def map[S](f: (T) => S): Future[S]|
  \item \verb|def flatMap[S](f: (T) => Future[S]): Future[S]|
  \item \verb|def filter(p: (T) => Boolean): Future[T]|
\end{itemize}



Importantly, since the above methods are provided, built-in Scala support for
for-comprehensions comes along for free.
...
Futures in Scala also come with a number of methods specialized on handling
failures by providing alternate operations in the event of a failure.
...
asynchronous operations to future values. We alluded to the
possibility of chaining . Such chaining amounts to buliding up a directed
acyclic graph (DAG) of computations.
