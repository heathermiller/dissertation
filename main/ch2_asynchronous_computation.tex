...
Finally, we present how FlowPools are able to overcome practical issues, such as
...
Nodes in this DAG are data containers which are first class
...
% Outside of academia, futures have become widespread in industry, with
% implementations (often blocking) in many languages such as
% JavaScript~\cite{JSFuturesCujo, JSFuturesJQuery, JSFuturesAngular},
% Python~\cite{PythonFutures}, Swift~\cite{SwiftFutures}, Ruby~\cite{RubyFutures},
% Dart~\cite{DartFutures}, Clojure~\cite{ClojureFutures}, and
% Racket~\cite{RacketFutures}, amongst others.
