...
% Though, often, serializing arbitrary data is a messy endeavor; one must either write custom serialization code by hand, often resulting in code duplication which doesn't scale, in the interest of performance (or the other way around, less boilerplate for poor performance), or one must have to contend with cryptic runtime exceptions which have propagated to surprising and confusing points of application code.
...
acknowledged as having a high overhead \cite{Jaguar, Carpenter1999}, with
...
Akka~\cite{Akka}, Spark~\cite{Spark}, SCADS \cite{Armbrust2009}, and
...
This chapter takes a step towards more principled open programming through a new
