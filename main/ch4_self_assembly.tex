\chapter{Static and Extensible Datatype Generic Programming}
\label{ch:self-assembly}

In the previous chapter, we covered {\em object oriented picklers}, and we had a
glimpse of an associated mechanism for the automatic generation of these type
class-based  picklers. In this chapter, we generalize our generation technique
from picklers to arbitrary type class instances.

% Generative programming via ping-ponging between macro expansion and a generated
% implicit lookup with then again can invoke implicit lookup.
...
% methods; \verb|toString|, \verb|equals|,
% \verb|clone|, and \verb|hashCode|.
...
% implements a \verb|Serializable| interface then instances of
...
% \verb|toString| or \verb|equals|) should be implemented
...
% embraced them~\cite{Oliveira2010,ScalaCollections,ScalaPickling}.
...
% providing operations such as \verb|equals| or
% \verb|toString|. It would be advantageous
...
%   system}~\cite{Burmako2012, Burmako2013} and it allows generating \emph{efficient} code~\cite{ScalaPickling}.  It
...
% framework for Scala~\cite{ScalaPickling} as a generic function (or LLE). The
...
\verb|toString|, \verb|equals|, \verb|clone|, and
\verb|hashCode|. Serialization, on the other hand, is also an
...
in Java is opt-in; if a class implements a \verb|Serializable| interface
...
flexible.
% An alternative approach to deal with functionality defined over a
% large set of types is to use \emph{type classes}~\cite{WadlerTypeClasses}.
%Library-based approaches like those using type
%classes~\cite{WadlerTypeClasses} are a lot more flexible.
...
\verb|toString| or \verb|equals|) are implemented together with the
...
them~\cite{Oliveira2010,ScalaCollections,ScalaPickling}.

When comparing type classes to baked-in functionalities (\eg Java
serialization), it's clear that an approach for adding functionality based on
type classes is more general, since most any functionality (including
serialization) can be modeled as a type class. However, an approach based on
type classes is not without challenges. To provide functionality across a large
number of types, users are required to implement many type class instances
manually, one by one. To reduce this vast amount of boilerplate, there have been
a number of proposals for \emph{datatype-generic programming}
(DGP)~\cite{ComparingGPHaskellRodriquez, ComparingGPHaskellHinze}.

DGP is an advanced form of \emph{generic programming}~\cite{GP}, where generic
functions can be defined by inspecting the structure of types. DGP approaches
are typically library-based, and as such they typically introduce many run-time
representations, thus typically incurring significant performance
penalties~\cite{TemplateYourBoilerplate}. Furthermore, the vast majority of DGP
approaches has been developed for Haskell, and are thus fundamentally limited
when ported to mainstream OO languages, due to their lack of support for
subtyping or object identity. A DGP approach appropriate for use in Scala must
account for such features.

% While there are efforts to use
% compiler inlining optimizations~\cite{OptimizingDGP}, it's difficult to
% predict when they'll kick-in, so normal programmers cannot rely on
% them.
% \footnote{BRUNO: I think there is too much detail here
%   discussing the limitations of DGP. Reader need to be aware that
%   there are library-based approaches...}

% Language integrated approaches (such as Java serialization) and approaches like type classes, are at

Baked-in compiler-based approaches to adding functionality is at odds with
library-based approaches using type classes. On the one
hand, language-integrated approaches can be more powerful in the sense that they
can do a great deal of static analysis, and because they are so specialized,
typically require no boilerplate to programmers. However this is done at the
cost of customizability and extensibility -- users typically can't override or
customize statically-added behavior. On the other hand, with type class-based
approaches, one must contend with an enormous amount of boilerplate or pay a
non-negligible performance penalty\footnote{Some approaches trade type-safety
for performance~\cite{TemplateYourBoilerplate}.}. In all cases, however, type
class-based approaches offer no way to statically restrict runtime behavior.

Perhaps the most important limitation of DGP approaches in the context of
mainstream languages is the lack of support for pervasively used object-oriented
features such as subtyping and object identity, which so far have not been
addressed, except for specialized functionality~\cite{ScalaPickling}.

% \footnote{BRUNO: Two important
%   points here -- restricting run-time behaviour (properties), and lack
% of support for OO features -- are mentioned in passing in a couple of
% sentences. I don't think this exposes well these ideas and why they
% are important/relevant.}

\subsection{Design Constraints}

This chapter details an approach that strikes a sweet spot in the design space.
...
The pickling framework, scala/pickling~\cite{ScalaPickling}, presesnted in the
previous chapter, sought to achieve many of these goals for one particular
application: serialization.
% In our previous work on scala/pickling~\cite{ScalaPickling}, we achieved all of these goals, except for
% generality.
% \footnote{BRUNO: I am not sure if this is a good way to promote this
%   work. It essentially gives the impression that only generic
%   properties are a contribution of this work: all the rest has been
%   done already by scala/pickling. It even makes it look
%   like as generic functions were already accounted for in Scala
%   Pickling (since you talk about generic functions in the previous
%   bullet points).}
...
This chapter presents {\em Self Assembly}, a general technique or pattern for:
\smallskip
\begin{itemize}[itemsep=1pt,topsep=0pt]
  \item Defining generic operations or properties that operate over a large
  class of types with little boilerplate and good performance (these operations
  are statically generated). Importantly, the technique supports many features
  of mainstream OO languages such as subtyping, object identity, and separate
  compilation.

  \item Defining additional lightweight static type checking via \emph{generic
  properties}. Such lightweight static checks can guarantee that a certain
  property (checked by tying together other static anaysis frameworks with the
  help of type classes), \eg deep immutability, holds. In this case, if a class
  is immutable, the immutability checker generates a type class instance for
  that class, which certifies that property.
\end{itemize}

% This chapter detailed a general mechanism, called \sselfassembly, for
...
% \emph{lightweight static extensions} (LLEs). LLEs allow users to define
...
% % Importantly, this migration ensures \selfassembly is
% % flexible enough to be adopted on a large scale in a production setting.
%
% Finally, we also show a different application of LLEs: \emph{generic
...
\begin{itemize}[itemsep=1pt,topsep=0pt]
  ...
  \item {\bf Self-Assembly}, a general technique for defining generic operations
  or properties that operate over a large class of types that requires little
  boilerplate; shares the extensibility and customizability properties of type
  classes; and, due to compile-time code generation, provides high performance. It
  allows defining generic functions in a statically type-safe way.

  \item {\bf A full-featured DGP approach for OOP.} \selfassembly enables the
  ...
  \item {\bf Support for generic properties.} \selfassembly enables the definition
  of custom lightweight static type checks to guarantee that certain static
  properties hold at runtime, \eg immutability.

    % \footnote{Bruno: I think here you should
    %   only claim generic properties; DGP/generic-functions are already
    %   claimed in previous bullet point.}
  ...
  \selfassembly doesn't require any extension to the language or compiler.

  \item {\bf A case study on basing scala/pickling on \sselfassembly.} We evaluate
  the expressivity and performance of \selfassembly by porting a full-featured
  serialization framework, keeping the same published performance numbers while
...
definitions. The remainder of the chapter aims at showing how
...
{\bf Implicit Parameters.} In Scala, it is possible to select values
...
% \begin{comment}
\textbf{Implicit Conversions.} Implicit conversions can be thought of as
...
types.  If values of a type \verb|T| should provide a certain
...
\end{minted}
\cprotect\caption{\texttt|Show| type class and corresponding instance for
  integers.}
\label{fig:showtc}
\end{figure}
...
The Scala encoding of type classes is essentially a \emph{design pattern}~\cite{Gamma1995}:
...
Figure~\ref{fig:showtc}. The trait \verb|Show[T]| models a type
class that provides pretty printing functionality for some type
\verb|T| via a method \verb|show|.
...
the type class \verb|Show| where the generic type parameter \verb|T| is
instantiated to \verb|Int|. The \verb|object IntInstance| in
Figure~\ref{fig:showtc} models such instance in
Scala using regular objects. In that object, the \verb|show|
method takes an argument \verb|o| of type \verb|Int| an
invokes the \verb|toString()| method on \verb|o|.
...
%%This method takes an object \verb|o| of some generic type \verb|T| and
%%an instance \verb|showT| of type \verb|Show[T]| and applies
%%the \verb|show| method in \verb|showT| to \verb|o|.
%%With \verb|eshow| we could
...
In \verb|ishow| the idea is that the method takes two parameters,
with the last of these (\verb|showT|) being implicit. As we have
...
example if we wanted to use \verb|show| on integers we could
...
Provided that an \verb|implicit| value of type
\verb|Show[Int]| is in the implicit scope (for example
\verb|IntInstance| from Figure~\ref{fig:showtc}), the second
...
bounds, instead of writting \verb|ishow| we could write:
...
\verb|show| can be read as a generic method where the generic
type argument must be an instance of \verb|Show|.
A small problem with context bounds there is no parameter name to be used in the
definition of \verb|show|. However, it is possible to
\emph{query} the implicit scope for a value of a certain type
using a simple auxiliary method called \verb|implicitly|:
...
From the client perspective, using \verb|show| is similar to
using \verb|ishow|.
...
\end{minted}
\cprotect\caption{Trees of integers and corresponding \verb|Show|
  instance.}
\label{fig:trees}
\end{figure}
...
trees in Scala using \emph{case classes}~\cite{Match} and \emph{sealed
  traits}. The keyword \verb|sealed| in Scala means that the
...
Together with case classes this allows modeling \emph{algebraic
  datatypes}, which are a well-known concept from functional programming.
The \verb|Tree| trait is the type of trees. The case class
\verb|Fork| models the binary nodes of the tree, wheres the case
class \verb|Leaf| models the leaves containing an integer value.

To define pretty printing for \verb|Tree| using the
\verb|Show| type class we create an \verb|object TreeInst|.
This object provides a definition for the
\verb|show| method that pattern matches on the
two tree constructors (cases) of \verb|Tree|. The implementation
of the two cases is unremarkable: both cases print the
constructors names and the arguments.

A simple test program illustrating the use of \verb|TreeInst|
is shown next. The value \verb|tree| defines a simple tree and the
definition \verb|test3| pretty prints that tree.
...
parametrized by some element type \verb|A|. The type these trees
is \verb|PTree[A]| and there are two types of nodes:
\verb|Branch| nodes with an element of type \verb|A|
and two branches; and \verb|Empty| nodes with no content.
...
\end{minted}
\cprotect\caption{Parametrized trees and corresponding \verb|Show|
  instance.}
\label{fig:ptrees}
\end{figure}

Like other types it is possible to define an instance (\verb|PTreeInst|)
for the type \verb|PTree[A]|. However in order to pretty print
such trees it is necessary to know how to print the elements of type
\verb|A| as well. To accomplish this we require that the
generic type parameter \verb|A| has a \verb|Show| instance
using a context bound.  To print the elements in the
\verb|Branch| case, the instance can be retrieved from the implicit
scope using \verb|implicitly| and then used to print the element.
...
for any element type that has a \verb|Show| instance. For example:
...
\noindent Here \verb|ptree2| has elements of type
\verb|PTree[Tree]|. To print \verb|ptree2| the instance for
\verb|PTree| is used twice: once for values of type
\verb|PTree[PTree[Tree]]|; and another time for values of
type \verb|PTree[Tree]|. In fact it is possible to use
...
%Both \verb|ishow| and \verb|show|
%provide a convenient way for some client code to invoke the pretty
%printing functionality on some values of type \verb|T|.
...
In the type-safe subset of macros that we consider in this chapter,
...
\end{minted}
  \cprotect\caption{Implementing the \verb|Show| type class using \sselfassembly.}
...
section~\ref{sec:self-assembly-basic-usage}, we outline the basic usage of the
\selfassembly library, which allows defining type classes desired in a way
where the required boilerplate for defining such type classes is automatically generated.
...
\label{sec:self-assembly-basic-usage}
...
\verb|Tree| type in Figure~\ref{fig:trees}.
%\begin{minted}{scala}
%sealed trait Tree
%case class Fork(left: Tree, right: Tree)
% extends Tree
...
\begin{figure}
\centering
\includegraphics[width=0.6\columnwidth]{basic-generation.pdf}
...
Sections~\ref{sec:basic-self-assembly}-\ref{sec:oo} were focused on generic
...
scala/pickling~\cite{ScalaPickling}). Interestingly, the implementations of queries and
transformations share a substantial number of generic building blocks.


\section{Generic Properties: Custom Lightweight Static Checks}
\label{sec:language-extensions}

In this section we show how our approach supports the definition of custom
lightweight static checking, similar to pluggable type system extensions, that
go beyond object-oriented DGP as discussed in the previous sections. In
particular, the \selfassembly library allows defining generic type-based
properties that can be checked by the existing Scala type checker.
...
The enabled static checks are lightweight in the sense that they cannot
...
supports added checking such as (transitive) type-based immutability checking,
...
implicits and present a high-level overview of our added lightweight static checks. We then show
...
To evaluate both expressivity and performance, we have ported the pickling framework presented in the previous chapter, scala/pickling~\cite{ScalaPickling}, to
...
hosting platform GitHub, the project has more than 630 ``stars''.
...
\textbf{Framework} & \textbf{Performance Change} & \textbf{LOC reduction} \\ % Column names row
...
\caption{Results of porting scala/pickling to \selfassembly} % Table caption, can be commented out if no caption is required
\label{tab:results} % A label for referencing this table elsewhere, references are used in text as \ref{label}
% \vspace{-1mm}
\end{table}

In terms of efficiency, \selfassembly compares favorably to the original
library: execution time of the ``Evactor'' benchmark~\cite{ScalaPickling} remains
...
Haskell (TH)~\cite{TemplateYourBoilerplate} to do DGP are closest to our work.
The use of TH is very often motivated by performance considerations, to avoid the
costs of run-time type representations.
However, published proposals using TH are based on its {\em untyped} macro
system. (TH itself has recently been upgraded to allow type-safe macros.)
Although type errors are still detected at compile time even using the untyped
system, they are given in terms of the generated code instead of the macro
code. In \selfassembly we do not need to make such a trade-off, because we
only use the type-safe subset of Scala’s macros (apart from a small, internal
trusted core, as is common in DGP approaches).

% However Template Haskell is an
...
capabilities. Furthermore, \selfassembly provides lightweight added type checks,
which can cannot
...
%, we use the standard Scala 2.11
% distribution.
...
This chapter detailed a general mechanism, called \sselfassembly, for defining
generic operations or properties that operate over a large class of types with
little boilerplate and good performance, and for defining additional lightweight
static typechecking via generic properties This mechanism has the extensibility
and customization advantages of type classes; and it has the automatic
implementation advantages of mechanisms like Java's serialization mechanism.
The key idea is to provide automatic implementations of type classes using
type-safe macros. This allows programmers to define their own generic
functionality, such as serialization, pretty printing, or equality; and it also
allows the definition of generic properties such as immutability checking. To
demonstrate the usefulness of \selfassembly in practice, we implemented an
industry-ready serialization framework for Scala.
